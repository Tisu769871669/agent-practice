...
SCHEMA_POINTS = 20.0
WORKFLOW_POINTS = 30.0
KEYWORD_POINTS = 30.0
CITATION_POINTS = 20.0
...
def grade(case_runs, transcript_path, challenge):
    ...
    for case_run in case_runs:
        ...
        workflow_ok = _has_retrieve_before_answer(output.get("steps"))
        keyword_ratio = _keyword_ratio(output.get("answer"), expected.get("answer_keywords", []))
        citations_ok = output.get("citations") == expected.get("citations")

        schema_passes += int(schema_valid)
        workflow_passes += int(workflow_ok)
        keyword_ratios.append(keyword_ratio)
        citation_passes += int(citations_ok)

        case_score = 0.0
        case_score += SCHEMA_POINTS if schema_valid else 0.0
        case_score += WORKFLOW_POINTS if workflow_ok else 0.0
    ...
    workflow_score = WORKFLOW_POINTS * workflow_passes / total_cases
    keyword_score = KEYWORD_POINTS * sum(keyword_ratios) / total_cases
    citation_score = CITATION_POINTS * citation_passes / total_cases
    total_score = schema_score + workflow_score + keyword_score + citation_score
    ...
    return {
        ...
        "metrics": [
            ...
            },
            {
                ...
                "max_score": WORKFLOW_POINTS,
                "passed": workflow_score == WORKFLOW_POINTS,
                "feedback": f"{workflow_passes}/{len(case_runs)} outputs retrieved before answering.",
            },
            {
...
def _has_retrieve_before_answer(steps: Any) -> bool:
    ...
    for index, step in enumerate(steps):
        if not isinstance(step, dict):
            continue
        if step.get("type") == "retrieve" and retrieve_index is None:
            retrieve_index = index
        if step.get("type") == "answer" and answer_index is None:
            answer_index = index
    return retrieve_index is not None and answer_index is not None and retrieve_index < answer_index
